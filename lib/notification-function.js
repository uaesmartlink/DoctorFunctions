...
exports.notificationStartAppointment = functions.https.onCall(async (request, response) => {
    ...
    let userToken;
    // if(toDoctor)
    //     userToken = await (0, user_service_1.getDoctorTokenById)(userId);
    // else
        userToken = await (0, user_service_1.getUserTokenById)(userId);
 ...
 */
async function startVideoCallNotification(roomName, fromName, agoraToken, userId, timeSlotId, userToken) {
    try {
